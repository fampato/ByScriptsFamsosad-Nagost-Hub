...
LeftGroupBox:AddToggle('AutoFarmToggle', {
    ...
    Tooltip = 'Auto Farm Level', -- Information shown when you hover over the toggle
})
Toggles.AutoFarmToggle:OnChanged(function(value)
    _G.Auto_Farm_Level = value    
end)
Toggles.AutoFarmToggle:SetValue(false)
LeftGroupBox:AddToggle('AutoNewWorldToggle', {
    Text = 'Auto New World',
    Default = true, -- Default value (true / false)
    Tooltip = 'Auto New World', -- Information shown when you hover over the toggle
})
Toggles.AutoNewWorldToggle:OnChanged(function(value)
...
Toggles.AutoNewWorldToggle:SetValue(false)
LeftGroupBox:AddToggle('AutoThirdWorldToggle', {
    Text = 'Auto Third World',
    Default = true, -- Default value (true / false)
    Tooltip = 'Auto Third World', -- Information shown when you hover over the toggle
})
Toggles.AutoThirdWorldToggle:OnChanged(function(value)
...
Toggles.AutoThirdWorldToggle:SetValue(false)
...
RightGroupBox:AddDropdown('SelectWeaponDropdown', {
    Values = WeaponList,
    Default = value, -- number index of the value / string
    Multi = false, -- true / false, allows multiple choices to be selected

    Text = '',
    Tooltip = 'Weapon / Combat', -- Information shown when you hover over the textbox
})
Options.SelectWeaponDropdown:OnChanged(function(value)
    _G.Select_Weapon = value
end)
Options.SelectWeaponDropdown:SetValue(false)
local MyButton = RightGroupBox:AddButton('Refresh Weapon', function()
    for i,v in pairs(game.ReplicatedStorage.Remotes.CommF_:InvokeServer("getInventory")) do
        SelectWeaponDropdown:Remove(v.Name)
    end
    for i,v in pairs(Melee) do
        SelectWeaponDropdown:Remove(v)
    end
    for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
        SelectWeaponDropdown:Remove(v.Name)
...
MyButton:AddTooltip('Refresh Weapon')
...
Library:SetWatermark('Nagos Hub Blox Fruit 2022 Scripts Pc By:Famsadboy')
...
getgenv().ToTarget = function(p)
    task.spawn(function()
        pcall(function()
            ...
            local B = TweenInfo.new((p.Position-game.Players.LocalPlayer.Character.Root.Position).Magnitude/340,Enum.EasingStyle.Linear)
